...
class complex:
	...
	def __add__(self, next):
		return complex(self.x+next.x, self.y+next.y)
	
	def __sub__(self, next):
		return complex(self.x-next.x, self.y-next.y)
	
	def __mul__(self, next):
		return complex(self.x*next.x-self.y*next.y, self.x*next.y+self.y*next.x)
	
	def __truediv__(self, next):
		try: 
			return self.__mul__(complex(next.x, -1*next.y)).__mul__(complex(1.0/(next.mod().x)**2, 0))
		except ZeroDivisionError as e:
			print (e)
			return None
	def mod(self):
		return complex(pow(self.x**2+self.y**2, 0.5), 0)
...
A = complex(float(a),float(b))
B = complex(float(x),float(y))
